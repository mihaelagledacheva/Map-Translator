    ...
    cv::Mat *output_image)
{
    ...
    for (int i = 0; i < vd.num_cells(); i++) {
        ...
        cv::drawContours((*output_image), contours, -1, color, cv::FILLED);
    }
    cv::Mat mask = cv::Mat::ones(output_image->size(), CV_8UC1) * 255;
    std::vector<std::vector<cv::Point>> borders;
    borders.emplace_back((*border));
    cv::fillPoly(mask, borders, cv::Scalar(0));
    cv::bitwise_xor(mask, (*output_image), (*output_image));
}
    ...
    const std::vector<cv::Point> *contour)
{
    ...
    for (int i = 0; i < ITERATIONS; i++) {                                  // iterate n times:
        d.add_point(&points, &segments, &vd, input_image);                  // add a point to the current set of points on the current vd of the current image
        vd.clear();                                                         // clear the current vd
        construct_voronoi(
            points.begin(), points.end(), 
            segments.begin(), segments.end(), &vd);                         // reconstruct it with the updated set of points
    }
    d.create_image(&points, &segments, contour, input_image, output_image);
}
